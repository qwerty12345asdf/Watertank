...
//This is the last code implements most of what needs to be done 

//rf integration
RH_ASK driver(2000,15,2); // ESP8266 or ESP32: do not use pin 11 gpio 2 is used as the transmitter pin i.e D4
int motorstate =0;//initialize motor state to off

//Time variables with ntp settings
String on_time,off_time;
int on_time_c,off_time_c;
int h_on,m_on;
int h_off,m_off;
String s_time_date ="";
// NTP Servers:
...
wifiManager.autoConnect("WaterTankConnect");
...
javaScript="<SCRIPT>\n";
  ...
  javaScript+="  var x7 = xmlResponse.getElementsByTagName('pump');\n";
  javaScript+="   document.getElementById('pump').innerHTML =x7[0].childNodes[0].nodeValue;\n";
  javaScript+="  var x8 = xmlResponse.getElementsByTagName('servo');\n";
  javaScript+="   document.getElementById('servo').innerHTML =x8[0].childNodes[0].nodeValue;\n";
...
//    webSite+="&nbspCurrently: <a id='c_motor'> </a> ";
     webSite +="<br> PUMP CONTROL :<form action='http://"+IPaddress+"' method='GET'><input name='motorstate' type='hidden' value='1'><input type='submit' value='PUMP ON' ></form>"; 
     webSite +="<form action='http://"+IPaddress+"' method='GET'><input name='motorstate' type='hidden' value='0'><input type='submit' value='PUMP OFF' ></form>"; 
     webSite+="<a id='pump'></a>";
     webSite +="<br><br> DISTRIBUTION SERVO CONTROL :<form action='http://"+IPaddress+"' method='GET'><input name='servostate' type='hidden' value='1'><input type='submit' value='DISTRIBUTION ON' ></form>"; 
     webSite +="<form action='http://"+IPaddress+"' method='GET'><input name='servostate' type='hidden' value='0'><input type='submit' value='DISTRIBUTION OFF' ></form>";  
     webSite +="<a id='servo'></a>";
     webSite += "</body>";
     webSite += "</html>";
}
...
void handleWebsite() {
  ...
  if (server.args() > 0 ) { // Arguments were received
    for ( uint8_t i = 0; i < server.args(); i++ ) {
      ...
      timeSplitting();
      on_time_c=timeCalculate(h_on,m_on);
      off_time_c=timeCalculate(h_off,m_off);
      if (server.argName(i) == "motorstate") {
        Serial.print("motor state received is : ");
        Serial.println(server.arg(i));
        motorstate = server.arg(i).toInt();
        // e.g. range_maximum = server.arg(i).toInt();   // use string.toInt()   if you wanted to convert the input to an integer number
        // e.g. range_maximum = server.arg(i).toFloat(); // use string.toFloat() if you wanted to convert the input to a floating point number
      }
      if (server.argName(i) == "servostate") {
        Serial.print("servo state received is : ");
        Serial.println(server.arg(i));
        servostate = server.arg(i).toInt();
        // e.g. range_maximum = server.arg(i).toInt();   // use string.toInt()   if you wanted to convert the input to an integer number
        // e.g. range_maximum = server.arg(i).toFloat(); // use string.toFloat() if you wanted to convert the input to a floating point number
      }
...
//////////////////Control servo and motor xml functions///////////////////////
//they pole the variables int motorstate and int servostate and return if on or off 
String motordisplay(){
if(motorstate==1){
  return "pump is on";
}
...
String servodisplay(){
if(servostate==1){
  return "distribution valve is on";
}
else if(servostate==0){
  return "distribution valve is off";
}
else {
  return "fetching data";
}
...
////////////////////////////Daily function/////////////////////////////////
void dailyCheck(){
  int time_now=(hour()*100)+minute();
  
  if(time_now>=on_time_c && time_now<off_time_c){
    Serial.println("motor works now my dear friend:");
    Serial.println("on time="+String(on_time_c)+" off time="+String(off_time_c));  
  }
  else {
 ...
 // servoOnOff();//calls the servoOnOff function to turn on or off the servo  
}
void timeSplitting(){
...
int timeCalculate(int h_c,int m_c){ // all variable related to this calculation will have and _c 
  int timeCalculated_c=(h_c*100)+m_c;//timeCalculated_c=0816(String).//timeCalculated=
  return timeCalculated_c;
}
void motorOnOff(){
...
void servoOnOff(){
    //checks if the servostate variable has been set to what state and sends appropriate message via rf.  
      if(servostate == 1){
  if(myservo.read()==120){
  return;
  }
 ...
 void waterCheck(){
  //this is just to replace the long code in the dailyCheck() function
  if(percentage_water_in_tank<upper_level_percentage_water_in_tank){
    motorstate=1;//turns on motor by setting the motor state function
    Serial.println("low water");
   }
  else if (percentage_water_in_tank>upper_level_percentage_water_in_tank){
     motorstate=0;//turn off the water pump
