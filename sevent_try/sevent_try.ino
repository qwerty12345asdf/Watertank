...
String on_time,off_time,default_on_time,default_off_time; 
String s_time_date ="";
// NTP Servers:
...
long duration;
int distance;
float total_height,default_total_height;//this variable needs to be exposed to the web-page so that it can be changed.
float distance_waterlevel,default_distance_waterlevel;//indicates the distance between the ultrasonic sensor and the waterlevel.
float percentage_water_in_tank;
int upper_level_percentage_water_in_tank,default_upper_level_percentage_water_in_tank;//should also be expose to the internet for change
int lower_level_percentage_water_in_tank,default_lower_level_percentage_water_in_tank;//expose to the internet
...
Serial.println("Wifi connected");
  ...
  //strating mdns domain name 
   if (!MDNS.begin("watertank")) {             // Start the mDNS responder for watertankcontrol.local
...
void buildWebsite(){
  ...
  webSite +=javaScript;
   ...
   /* webSite += "Height of water-tank(in cm):<input type='number' name='total_height' max='2000' required><br>";
    ...
    webSite +="<br>Control output valve:&nbsp<input type='button' onclick='turnOnServo()' value='Turn ON' > ";
    webSite+="&nbsp <input type='button' onclick='turnOffServo()' value='Turn OFF'> ";
